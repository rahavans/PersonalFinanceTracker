...
class PersonalFinanceTracker:
    ...
    def update_table(self):
        ...
        for col in ["Date", "Type", "Description", "Amount"]:
            self.tree.column(col, width=tkFont.Font().measure(max(self.df[col].astype(str), key=len)) + 10)
    ...
    def delete_selected_expense(self):
        ...
        for item in selected_items:
            index = int(item.split("I")[1])  # Extract index from item ID
    ...
    def make_excel(self):
        ...
        tk.messagebox.showinfo("Export Successful", f"Data exported to {excel_filename}")
